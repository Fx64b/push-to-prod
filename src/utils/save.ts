...
export function encodeSave(state: PersistedState): string {
  ...
  // Layer 2: position-dependent byte rotation
  const p2 = _rot(p1, true)

  // Layer 3: XOR with stream keyed from CRC + seed 1 (different from layer 1)
  const p3 = _xr(p2, _ks(_s1 ^ crc))

  // Packet: [magic(4)] [crc(4 big-endian)] [payload]
...
export function validateState(raw: unknown): PersistedState | null {
  ...
  return {
    ...
    productName:
      typeof s.productName === 'string' ? s.productName.slice(0, 50) : 'My Product',
  }
}

  ...
  return (
    <div className="flex flex-col items-center gap-2 font-mono">
      <div className="text-[10px] text-gh-muted text-center">
        ...
        <br />
        Earn <span className="text-gh-purple font-bold">+{tokensToEarn} Legacy Token{tokensToEarn !== 1 ? 's' : ''}</span>
        {' '}(have: {legacyTokens})
      </div>
      ...
      {confirming && (
        <p className="text-[10px] text-gh-red">
          Click again to confirm — all progress resets!
        </p>
      )}

      <div className="text-[10px] text-gh-muted text-center">
        Current LOC: <span className="text-gh-green">{formatLOC(totalLoc)}</span>
        {' '}/ threshold: <span className="text-gh-muted">{formatLOC(PRESTIGE_THRESHOLD)}</span>
      </div>
